...
| `PodBridge.App` | WPF tray-first UI, view models, notifications, settings, and the composition root (DI, background host). Includes the **About window** (`AboutWindow` + `AboutViewModel`) — the app's first non-tray window, opened from the tray "About" entry, which renders the Core `ProductInfo` branding/disclaimer/license/audio constants plus the running app version and the shipped `THIRD-PARTY-NOTICES.md`. |
