...
4. **Noise-control / gesture (Tier 2, opt-in):** `App` command →
   ...
   an "enable advanced tier" affordance (opens the docs) rather than failing silently; the
   app stays `asInvoker`. This flow is now implemented end-to-end (functional behaviour on
   real hardware remains the Tier-2 manual smoke test).
