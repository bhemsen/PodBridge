...
| --------- | -------------- |
| `PodBridge.Core` | Platform-neutral domain: the clean-room AAP module (`AapProtocol` frame builders + notification parsers) covering **noise-control** (`BuildSetNoiseControl`/`TryParseNoiseControlNotification` + `NoiseControlController` optimistic-set/echo-confirm/timeout-revert logic over `IAapTransport`, with `DeviceState.NoiseControl`/`NoiseControlAvailable`, `NoiseControlMode`, and the `NoiseControlSupport` Adaptive model gate) and **press-and-hold gesture remap** (`BuildSetPressAndHoldGesture`/`TryParsePressAndHoldGestureNotification` for the per-bud ClickHoldMode `0x16` frame — right=`data1`, left=`data2` — plus the `GestureAction` enum (Noise Control `0x01`/Siri `0x05` only; single/double presses are Apple-fixed and unexposed) and the per-bud `GestureConfiguration` model, the `GestureSupport` model gate (press-and-hold offered on the AirPods Pro 2 reference model only; broad matrix is Phase 8), the `GestureRepushController` re-push-on-reconnect policy, and the `GestureSettingsController`/`GestureAvailability` settings decision+apply logic that resolves the driver-absent/model-unsupported/available states and persists via `IGestureConfigStore` + writes via the re-push policy), Apple-Continuity BLE parser (`ContinuityParser` → `BleAdvertisement` in, `DeviceState` battery/charging/in-ear out), the Phase-8 model registry (`IModelRegistry`/`ModelRegistry`, backed by the clean-room `AppleModelIdentifier` per-model shape mapper) resolving an identified `AirPodsModel` to its `AirPodsModelInfo` shape (dual-bud vs single unit, battery-reporting case, in-ear vs head detection) for the vision's six supported models, with any other model — including Phase-2 `AirPodsModel` values outside that six-model scope — degrading to the labelled generic "Unknown AirPods" `AirPodsModelInfo` fallback (issue #52), the Phase-8 capability provider (`Capabilities/ICapabilityProvider`/`CapabilityProvider`, backed by the static `CapabilityMatrix` keyed by **(model, firmware-major)**) — the single Core authority that decides which features are offered: **Tier-1** features (`Tier1Feature`) gate on the BLE-derived model shape only (never firmware, never driver — e.g. AirPods Max hides case battery), while **Tier-2** features (`Tier2Feature` noise-control/gesture-remap/conversation-awareness) gate on **both** driver presence (via `IAapTransport.IsAvailable`) **and** the (model, firmware-major) capability, returning an honest `CapabilityDecision` (reason "requires the optional driver" / "not supported on this model" / "not supported on this firmware" / "supported"); the firmware-major axis is a documented no-op today (no host-requestable read exists — `0x001D` "Device Information" is a fragile accessory-initiated push — so an unreadable firmware-major falls back to the model-level Tier-2 capability and a Phase-6/7-supported model stays on, no regression; future firmware-varying refinements are `FirmwareRefinement` data edits, of which the shipped set is empty — docs/research/firmware-capabilities.md, issue #53), the connection-gated telemetry pipeline (`IDeviceStateProvider`/`DeviceStateTracker`), the auto play/pause engine (`AutoPlayPauseEngine`), the read-only audio-state surface (`IAudioStateReader` → `AudioState`(`CodecKind`, `MicMode`) + `AudioGuidanceEngine`), mic-policy engine, the branding/disclaimer/license constants (`Branding/ProductInfo` — coined name, "for AirPods" descriptor, not-affiliated disclaimer, Apache-2.0 id, honest audio note, docs link; guarded by the Tier-1 `ProductInfoTests`), the opt-in auto-start toggle contract (`Startup/IStartupToggle` → `StartupToggleState`; default OFF, guarded by the Tier-1 `StartupToggleTests`), the Phase-8 local **diagnostics** model (`Diagnostics/DiagnosticsSnapshot` — model, firmware-major, negotiated codec, tier, driver presence + honest `DriverSigningStatus`, the `ICapabilityProvider`-derived `CapabilityMatrixEntry` matrix, and address-masked `BleParseResult` history; assembled deterministically by `DiagnosticsSnapshotBuilder`/`IDiagnosticsSnapshotFactory` from the existing Core seams — the first real consumer of `ICapabilityProvider` — rendered to plain text by `DiagnosticsSnapshotFormatter`, recorded by the `IBleParseHistory`/`BleParseHistoryRecorder` masking recorder; no secret, no full Bluetooth address — issue #54), and the OS-boundary interfaces (`IBleScanner`, `IConnectionMonitor`, `IMediaController`, `IAudioStateReader`, `IAudioPolicy`, `IAudioSessionMonitor`, `IBluetoothRadioSource`, `IStartupToggle`, `IAapTransport`, the local diagnostics-writer boundary `IDiagnosticsExporter` → `DiagnosticsExportResult` (local file only, no network), and the advanced-tier opt-in install boundary `IAdvancedTierInstaller` → `AdvancedTierActionResult` plus the honest opt-in copy `AdvancedTier/AdvancedTierInfo`, guarded by `AdvancedTierInfoTests`). No OS/UI/P-Invoke. |
| `PodBridge.Windows` | Windows adapters implementing the Core interfaces: `WinRtBleScanner` (BLE advertisement watcher), `WinRtConnectionMonitor` (WinRT paired/connected detection), `WinRtBluetoothRadioSource` (`IBluetoothRadioSource` over `Windows.Devices.Radios`; reports the Bluetooth-radio power state so `BleScannerSupervisor` restarts the watcher on a radio toggle, degrading to "assume on" if radio enumeration faults), `WindowsMediaController` (GSMTC media-session pause/resume), `WindowsAudioStateReader` (read-only Core Audio codec + mic-mode read), `WindowsAudioPolicy` (NAudio + `IPolicyConfig`), `WindowsAudioSessionMonitor` (`IAudioSessionManager2`), `RunKeyStartupToggle` (opt-in auto-start over the per-user `HKCU\...\Run` key, default OFF, self-healing a stale path on launch and honouring a Task-Manager user-disable via `StartupApproved\Run`), the Phase-8 diagnostics/logging adapters `DiagnosticsExporter` (`IDiagnosticsExporter`: renders a `DiagnosticsSnapshot` and writes one timestamped text file under `%LOCALAPPDATA%\PodBridge\diagnostics`; local filesystem only, structurally proven to reference no network-capable type) and `RollingFileLoggerProvider` (a minimal custom `ILoggerProvider`/`ILogger` on `Microsoft.Extensions.Logging.Abstractions` — the app's **only** log sink, a rolling, size/age-capped local file under `%LOCALAPPDATA%\PodBridge\logs` (~10 MB / 7 days) whose runtime-mutable `MinLevel` backs the tray Debug toggle; no network sink), and — Tier 2 only — `DriverAapTransport` (talks to the KMDF driver) and `AdvancedTierInstaller` (implements `IAdvancedTierInstaller`: locates the shipped-separately `install-advanced-tier.ps1` and launches it **elevated** via ShellExecute `runas` on explicit user action — driver `pnputil` install + self-signed test-cert trust in one step; never elevates the app, never runs `bcdedit`). |
| `PodBridge.App` | WPF tray-first UI, view models, notifications, settings, and the composition root (DI, background host). The tray context menu is driven by per-feature controllers (`TrayStatusController`, `TrayBatteryController`, `TrayAudioController`, `TrayMicController`, the Phase-8 `TrayDiagnosticsController` for the "Export diagnostics" action — build snapshot → write local file → copy the same text to the WPF clipboard (the clipboard copy lives here, keeping the Windows adapter UI-framework-free) → honest notification — and the "Debug logging" toggle that flips the local `RollingFileLoggerProvider.MinLevel` at runtime, and — Tier 2, opt-in — `TrayNoiseControlController` for the "Noise control" submenu with the driver-absent honesty UX + Adaptive model gate). The composition root replaces the generic host's default log providers with the single local `RollingFileLoggerProvider` sink (no console, no network sink). Includes the **About window** (`AboutWindow` + `AboutViewModel`) — the app's first non-tray window, opened from the tray "About" entry, which renders the Core `ProductInfo` branding/disclaimer/license/audio constants plus the running app version and the shipped `THIRD-PARTY-NOTICES.md`, and carries the **opt-in auto-start-at-login toggle** (default OFF) that reads/sets `IStartupToggle`. Adds the **gesture-controls window** (`GestureSettingsWindow`, opened from the tray "Gesture controls…" entry) — the Tier-2, opt-in press-and-hold remap surface: it binds the Core `GestureSettingsController` + `IDeviceStateProvider` to per-bud action pickers, and when the driver is absent (Tier-1 default) replaces them with the **reused** Phase-6 driver-absent notice (`TrayNoiseControlController.UnavailableText`, no new signed-driver claim) plus the same explicit "Enable advanced tier" flow — never silently broken. |
| `driver/PodBridgeAAP` | Optional C / KMDF L2CAP-bridge driver exposing a user-mode device interface for AAP over PSM 0x1001. Ships **separately** (never bundled with the app exe, its own INF + `pnputil` install): `build-testsign.ps1` builds + self-signs it, and `install-advanced-tier.ps1` is the elevated opt-in install/uninstall (`pnputil` + importing/removing the self-signed test cert in Trusted Root CA / Trusted Publishers). It never enables test-signing (`bcdedit`) — that stays a documented manual user step. |
...
- Advanced-tier install/signing UX → the honest copy + boundary in Core
  ...
  never bundle the driver in the app exe and never auto-elevate the app.
- Packaging/distribution (release-1.0): PodBridge ships as a self-contained,
  single-file `.exe` per RID (`win-x64`/`win-arm64`) published straight from
  `PodBridge.App.csproj` (`dotnet publish`) and attached to a tagged GitHub
  Release by `.github/workflows/release.yml` — no MSIX, no Microsoft Store, no
  installer (superseded Phase-5 MSIX/Store work archived, issue #124; #38
  closed as superseded). Auto-start is `RunKeyStartupToggle` (`PodBridge.Windows`)
  over the per-user `HKCU\...\Run` key, replacing the retired MSIX
  `windows.startupTask`/`StartupTaskToggle`. The About window's disclaimer/
  license-notices are packaging-independent: `LICENSE`/`NOTICE`/
  `THIRD-PARTY-NOTICES.md` are embedded assembly resources
  (`PodBridge.App.csproj`), so the running exe is sidecar-free regardless of
  distribution channel. The Tier-2 driver stays on its own separate INF +
  `pnputil` opt-in install (`driver/PodBridgeAAP`), unaffected by this pivot.
